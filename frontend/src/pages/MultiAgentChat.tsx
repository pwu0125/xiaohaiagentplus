...
export default function MultiAgentChat() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setResults([]);
    try {
      const response = await discussWithAgents(topic.trim(), selectedAgentIds, context.trim() || undefined);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-slate-700 bg-slate-950/60 p-6 shadow-xl shadow-slate-950/20">
        ...
        {error ? (
          <div className="mt-6 rounded-3xl border border-red-500/20 bg-red-500/10 p-4 text-red-100">{error}</div>
        ) : null}
      ...
      <section className="space-y-4">
        ...
        {loading && results.length === 0 ? (
          <div className="rounded-3xl border border-slate-700 bg-slate-950/70 p-8 text-center text-slate-400">讨论中，请稍候…</div>
        ) : results.length === 0 ? (
          ...
          <div className="grid gap-4">
            {results.map((result) => (
              <article key={`${result.agent_id}-${result.timestamp}`} className="rounded-3xl border border-slate-700 bg-slate-950/80 p-6 shadow-sm shadow-slate-950/10">
